...
public class LunchMachineView extends javax.swing.JFrame {
    ...
    private void cmdCargarDeDiscoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdCargarDeDiscoActionPerformed
        ...
        if (seleccion == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                DiscoDuro usb = new DiscoDuro();
                FileReader archivo = new FileReader(fichero.getAbsoluteFile());
                BufferedReader entrada = new BufferedReader(archivo);
                entrada.readLine();
                String tamanio[] = usb.leerTamanio2CSV(fichero.getAbsolutePath());
                int tamanioEnteros[] = new int[tamanio.length];
                for (int i = 0; i < tamanio.length; i++) {

                    tamanioEnteros[i] = Integer.parseInt(tamanio[i]);
                }
                for (int i = 0; i < tamanioEnteros[0]; i++) {
                    almacenarDatos = entrada.readLine().split(",");
                    ingredientes.add(new Fruta(almacenarDatos[0], Integer.parseInt(almacenarDatos[2]), Integer.parseInt(almacenarDatos[1])));
                }
                for (int i = 0; i < tamanioEnteros[1]; i++) {
                    almacenarDatos = entrada.readLine().split(",");
                    ingredientes.add(new Proteina(almacenarDatos[0], Integer.parseInt(almacenarDatos[2]), Integer.parseInt(almacenarDatos[1])));
                }
                for (int i = 0; i < tamanioEnteros[2]; i++) {
                    almacenarDatos = entrada.readLine().split(",");
                    ingredientes.add(new Grano(almacenarDatos[0], Integer.parseInt(almacenarDatos[2]), Integer.parseInt(almacenarDatos[1])));
                }
                for (int i = 0; i < tamanioEnteros[3]; i++) {
                    almacenarDatos = entrada.readLine().split(",");
                    ingredientes.add(new Almidon(almacenarDatos[0], Integer.parseInt(almacenarDatos[2]), Integer.parseInt(almacenarDatos[1])));
                }
            } catch (Exception e) {
    ...
    private void cmdGuardarADiscoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdGuardarADiscoActionPerformed
        ...
        if (seleccion == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                FileOutputStream archivo = new FileOutputStream(fichero.getAbsoluteFile());
                PrintStream salida = new PrintStream(archivo);
                salida.println(restaurante.getNombresFrutas().length + "," + restaurante.getNombresProteinas().length + "," + restaurante.getNombresGranos().length + "," + restaurante.getNombresAlmidones().length);
                for(int i=0;i<restaurante.getIngredientes().size();i++){
                    salida.println(restaurante.getIngredientes().get(i).nombre+","+restaurante.getIngredientes().get(i).cantidadDisponible+","+restaurante.getIngredientes().get(i).precioCompra);
                }
